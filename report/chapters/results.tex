...
The usage of transfer learning also reduced the amount of time needed for training, with both models requiring fewer epochs
to achieve the peak result.
...
From the above figure, it is clear that neural networks vastly outperform the KNN classifier,
with pre-training greatly reducing the waste of good beans and the chance of a defective bean,
especially one with a high-impact defect, making it into the final product.

Considering the evaluation time and these metrics, the MobileNet architecture stands out as one with the most potential
in a real-world environment.
Its classification time would also allow it to be used in a high-throughput system, with the ResNet being the best choice for a roaster
willing to trade real-time classifications for the best possible results.
